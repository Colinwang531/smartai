...
namespace framework
{
	...
	{
		MediaGraph::MediaGraph()// : mediaFrameCallback{ NULL }, videoDisplayHwnd{ NULL }, userData{ NULL }
		{}
			...
			const std::string url, void* hwnd /* = NULL */, MediaFrameCallback callback /* = NULL */, void* userData /* = NULL */)
		{
			...
			if (ERR_OK == status)
			{
				status = createAVCaptureFilter();
				if (ERR_OK == status)
				{
// 					mediaFrameCallback = callback;
// 					videoDisplayHwnd = hwnd;
// 					this->userData = userData;
		...
		int MediaGraph::createAVCaptureFilter()
		{
			int status{ ERR_BAD_ALLOC };
			MediaFilterPtr mediaFilterPtr{ boost::make_shared<AVCaptureFilter>() };
			if (mediaFilterPtr)
			{
				status = mediaFilterPtr->createNewFilter(AVCaptureFilterID);
				if (ERR_OK == status)
				{
					mediaFilterGroup.insert(AVCaptureFilterID, mediaFilterPtr);
				}
			}
